# --- START OF COMPLETE app.py (Corrected & Improved v4) ---
...
def configure_logging(app):
    ...
    if not app.debug:
        ...
        db_handler = RotatingFileHandler(db_log_file, maxBytes=2*1024*1024, backupCount=3)
        ...
        socket_handler = RotatingFileHandler(socket_log_file, maxBytes=2*1024*1024, backupCount=3)
        ...
        app.logger.info('Application logging initialized for production')
    else:
        logging.getLogger('sqlalchemy.engine').setLevel(logging.INFO)
        logging.getLogger('socketio').setLevel(logging.DEBUG)
        logging.getLogger('engineio').setLevel(logging.DEBUG)
        app.logger.info('Application logging initialized for development (DEBUG)')
...
# Configuration SocketIO (Polling uniquement)
socketio = SocketIO(
    app,
...
class Participant(db.Model):
    ...
    def __repr__(self):
        return f'<Participant {self.prenom} {self.nom}>'
...
class Activite(db.Model):
    ...
    # --- CORRECTION INDENTATION date_relative ---
    @property
    def date_relative(self):
        ...
        if seconds < 5:
            ...
            return f"il y a {int(seconds)} seconde{'s' if int(seconds) >= 2 else ''}"
        elif seconds < 3600:
            ...
            if days > 7:
                try:
                    ...
                    return f"il y a {days} jour{'s' if days >= 2 else ''}" # Fallback
            else:
                return f"il y a {days} jour{'s' if days >= 2 else ''}"
    # --- FIN CORRECTION ---

    def __repr__(self):
        return f'<Activite {self.id} - Type:{self.type} Date:{self.date}>'
...
class User(UserMixin, db.Model):
    ...
    # activites relation is defined via backref in Activite model
...
def generate_ics(session, participant, salle=None):
    try:
        ...
        if salle and salle.lieu:
            description_lines.append(f"Adresse: {salle.lieu}")
...
def add_activity(type, description, details=None, user=None):
    try:
        ...
        db.session.commit()
        db.session.refresh(activite) # Recharger pour obtenir les relations et propriétés calculées
...
def services():
    try:
        services = Service.query.options(joinedload(Service.participants)).order_by(Service.nom).all()
        
        service_data = [{'obj': s, 'participant_count': len(s.participants)} for s in services]
        
        participants = get_all_participants() # Needed for add modal
...
def sessions():
    try:
        query = Session.query.options(joinedload(Session.theme), joinedload(Session.salle)).order_by(Session.date, Session.heure_debut)
...
def inscription():
    ...
    try:
        ...
        add_activity('inscription', f'Demande inscription: {participant.prenom} {participant.nom}',
                    f'Session: {session_obj.theme.nom} ({session_obj.formatage_date})', user=current_user)
...
def validation_inscription(inscription_id):
    ...
    try:
        ...
        if action == 'valider' and inscription.statut == 'en attente':
            ...
            add_activity('validation', f'Validation: {participant_name}',
            ...
            add_activity('refus', f'Refus: {participant_name}',
                        session_info, user=current_user)
            ...
            add_activity('annulation', f'Annulation: {participant_name}',
                        session_info, user=current_user)
...
def add_participant():
    ...
    try:
        ...
        add_activity('ajout_participant', f'Ajout participant: {participant.prenom} {participant.nom}',
                    f'Service: {participant.service.nom}', user=current_user)
        ...
        if redirect_session_id and action_after_add:
            ...
            if action_after_add == 'liste_attente' or session_obj.get_places_restantes() <= 0:
                position = db.session.query(func.count(ListeAttente.id)).filter(
                    ListeAttente.session_id == redirect_session_id).scalar() + 1
                ...
                add_activity('liste_attente', f'Ajout liste attente (ajout P.): {participant.prenom}',
                            f'S:{redirect_session_id}, P:{position}', user=current_user)
                ...
                add_activity('inscription', f'Demande inscription (ajout P.): {participant.prenom}',
                            f'S:{redirect_session_id}', user=current_user)
...
def participants():
    ...
    try:
        participants_raw = Participant.query.options(joinedload(Participant.service)).order_by(Participant.nom, Participant.prenom).all()
        ...
        for p in participants_raw:
            try:
                confirmed_count = db.session.query(func.count(Inscription.id)).filter(
                    Inscription.participant_id == p.id,
                    Inscription.statut == 'confirmé'
                ).scalar() or 0
                
                waitlist_count = db.session.query(func.count(ListeAttente.id)).filter(
                    ListeAttente.participant_id == p.id
                ).scalar() or 0
                ...
                participants_data.append({
                    'obj': p,
                    'inscriptions_count': confirmed_count,
                    'attente_count': waitlist_count,
                    'pending_count': pending_count
                })
            except Exception as e_inner:
                app.logger.error(f"Error processing participant {p.id} counts: {e_inner}")
                participants_data.append({
                    'obj': p,
        ...
        # Ajouter ListeAttente au contexte du template
        ...
        db.session.rollback()
        app.logger.error(f"Unexpected error loading participants: {e}", exc_info=True)
        flash("Erreur interne.", "danger")
        return redirect(url_for('dashboard'))
...
def update_participant(id):
    ...
    try:
        ...
        add_activity('modification_participant', f'Modif participant: {participant.prenom} {participant.nom}',
                   f'Service: {participant.service.nom}', user=current_user)
...
def delete_participant(id):
    ...
    try:
        # Vérifier s'il a des inscriptions confirmées
        ...
        if inscriptions_count > 0:
            flash(f'Impossible de supprimer "{participant.prenom} {participant.nom}", {inscriptions_count} inscription(s) confirmée(s).', 'danger')
            return redirect(redirect_url)
        
        # Supprimer les inscriptions en attente et listes d'attente
        ...
        flash(f'Participant "{nom_participant}" supprimé avec succès.', 'success')
    
    except (OperationalError, TimeoutError) as oe:
        db.session.rollback()
        app.logger.error(f"DB Pool/Conn Error delete participant {id}: {oe}")
        flash("Erreur connexion/pool DB.", "danger")
    
    except SQLAlchemyError as e:
        db.session.rollback()
        flash('Erreur DB suppression participant.', 'danger')
        app.logger.error(f"SQLAlchemyError delete participant {id}: {e}")
    
    except Exception as e:
        db.session.rollback()
        flash('Erreur inattendue lors de la suppression.', 'danger')
        app.logger.error(f"Error delete participant {id}: {e}")
...
def admin():
    ...
    try:
        themes = Theme.query.options(joinedload(Theme.sessions)).order_by(Theme.nom).all()
        
        sessions = Session.query.options(joinedload(Session.theme), joinedload(Session.salle)).order_by(Session.date, Session.heure_debut).all()
        
        services = Service.query.options(joinedload(Service.participants)).order_by(Service.nom).all()
        
        participants = Participant.query.options(joinedload(Participant.service)).order_by(Participant.nom).all()
        ...
        for s in sessions:
            try:
                ...
                if session_counts is None:
                    ...
                    session_counts = {'places_restantes': max(0, s.max_participants - inscrits_count)}
                
                if session_counts['places_restantes'] == 0:
                    total_sessions_completes += 1
            except Exception as count_err_sess:
                app.logger.warning(f"Could not calculate places for session {s.id} in admin stats: {count_err_sess}")
        ...
        db.session.rollback()
        app.logger.error(f"DB Pool/Connection Error loading admin: {oe}")
        flash("Erreur connexion/pool DB.", "danger")
        return render_template('error.html', error_message="Erreur connexion base de données."), 503
        ...
        db.session.rollback()
        app.logger.error(f"Unexpected error loading admin: {e}", exc_info=True)
        flash("Erreur interne.", "danger")
        return render_template('error.html', error_message="Erreur interne."), 500

# --- Route /salles (CORRIGÉE) ---
...
def salles():
    ...
    try:
        salles_raw = Salle.query.order_by(Salle.nom).all()
        
        salles_data = []
        
        for s in salles_raw:
            try:
                sessions_associees = Session.query.filter(Session.salle_id == s.id).options(
                    joinedload(Session.theme)).order_by(Session.date).all()
                
                salles_data.append({
                    'obj': s,
                    'sessions_count': len(sessions_associees),
                    'sessions_associees': sessions_associees
                })
            except Exception as e_inner:
                app.logger.error(f"Error processing salle {s.id} counts/sessions: {e_inner}")
                salles_data.append({
                    'obj': s,
                    'sessions_count': -1,
                    'sessions_associees': []
        ...
        db.session.rollback()
        app.logger.error(f"Unexpected error loading salles: {e}", exc_info=True)
        flash("Erreur interne.", "danger")
        return redirect(url_for('admin'))
...
def update_salle(id):
    ...
    try:
        ...
        if new_nom != original_nom and Salle.query.filter(
                Salle.nom == new_nom,
                Salle.id != id).first():
...
# --- Route /delete_salle (AJOUTÉE et CORRIGÉE) ---
...
def delete_salle(id):
    ...
    try:
        sessions_associees_count = db.session.query(func.count(Session.id)).filter(
            Session.salle_id == id).scalar()
        
        if sessions_associees_count > 0:
            flash(f'Impossible de supprimer "{salle.nom}", {sessions_associees_count} session(s) y sont encore associées.', 'danger')
            return redirect(redirect_url)
        ...
        db.session.rollback()
        app.logger.error(f"DB Pool/Conn Error delete salle {id}: {oe}")
        flash("Erreur connexion/pool DB.", "danger")
    
    except SQLAlchemyError as e:
        db.session.rollback()
        flash('Erreur DB suppression salle.', 'danger')
        app.logger.error(f"SQLAlchemyError delete salle {id}: {e}")
    
    except Exception as e:
        db.session.rollback()
        flash('Erreur inattendue lors de la suppression.', 'danger')
        app.logger.error(f"Error delete salle {id}: {e}")
...
def themes():
    ...
    try:
        themes_list = Theme.query.options(joinedload(Theme.sessions)).order_by(Theme.nom).all()
        
        return render_template('themes.html', themes=themes_list)
        ...
        db.session.rollback()
        app.logger.error(f"Unexpected error loading themes: {e}", exc_info=True)
        flash("Erreur interne.", "danger")
        return redirect(url_for('admin'))
...
def update_theme(id):
    ...
    try:
        ...
        if new_nom != original_nom and Theme.query.filter(
                Theme.nom == new_nom,
                Theme.id != id).first():
            flash(f"Nom '{new_nom}' déjà pris.", 'warning')
            return redirect(redirect_url)
...
def update_service(service_id):
    ...
    try:
        ...
        if new_nom != original_nom and Service.query.filter(
                Service.nom == new_nom,
...
def activites():
    ...
    try:
        ...
        query = Activite.query.options(joinedload(Activite.utilisateur)) # Eager load utilisateur
        ...
        app.logger.error(f"Unexpected error loading activities: {e}", exc_info=True)
        flash("Erreur interne.", "danger")
        return redirect(url_for('admin'))

# --- API Routes (Optimisation des comptes) ---
...
def api_sessions():
    ...
    for s in sessions:
        try:
            inscrits = db.session.query(func.count(Inscription.id)).filter(Inscription.session_id == s.id, Inscription.statut == 'confirmé').scalar() or 0
            attente = db.session.query(func.count(ListeAttente.id)).filter(ListeAttente.session_id == s.id).scalar() or 0
            places = max(0, s.max_participants - inscrits)
            salle_nom = s.salle.nom if s.salle else None
            salle_id = s.salle_id
            theme_nom = s.theme.nom if s.theme else "N/A"
            theme_id = s.theme_id
...
def api_single_session(session_id):
    try:
        ...
        result = {
            ...
            'places_restantes': max(0, session.max_participants - inscrits_count),
        ...
        app.logger.error(f"API Unexpected Error session {session_id}: {e}")
...
def api_participants():
    ...
    for p in participants:
        try:
            confirmed = db.session.query(func.count(Inscription.id)).filter(
                Inscription.participant_id == p.id,
                Inscription.statut == 'confirmé'
            ).scalar() or 0
            
            waitlist = db.session.query(func.count(ListeAttente.id)).filter(
                ListeAttente.participant_id == p.id
            ).scalar() or 0
...
def api_salles():
    ...
    for salle in salles:
        try:
            count = db.session.query(func.count(Session.id)).filter(Session.salle_id == salle.id).scalar() or 0
            result.append({
                'id': salle.id,
                'nom': salle.nom,
...
# --- ICS File Generation Route (inchangé) ---
...
def generer_invitation(inscription_id):
    try:
        ...
        add_activity('telecharger_invitation', f'Téléchargement invitation: {participant.prenom} {participant.nom}',
                   f'Session: {session.theme.nom} ({session.formatage_date})', user=current_user)
...
# === Database Initialization (inchangé) ===
def init_db():
    with app.app_context():
        try:
            ...
            if User.query.count() == 0:
                ...
                for service in services:
                    ...
                    username = username.replace('é', 'e').replace('è', 'e').replace('ê', 'e').replace('à', 'a').replace('â', 'a').replace('ô', 'o').replace('ù', 'u')[:20]
            ...
            if Activite.query.count() == 0:
                ...
                activites_data = [
                    {'type': 'systeme', 'description': 'Initialisation application', 'details': 'DB et données initiales créées'},
                    {'type': 'systeme', 'description': 'Création comptes utilisateurs', 'details': 'Admin et responsables créés', 'utilisateur_id': admin_user.id if admin_user else None}
                ]
...
if __name__ == '__main__':
    ...
    try:
        if is_production:
            ...
            socketio.run(app, host='0.0.0.0', port=port, use_reloader=False, debug=debug_mode, log_output=False, allow_unsafe_werkzeug=True)
    except Exception as e:
...
# --- END OF COMPLETE app.py (Corrected & Improved v4) ---        
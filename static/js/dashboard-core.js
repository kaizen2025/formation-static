 ...
 * Version: 2.0.0 - Refactorisé pour intégrer les correctifs et améliorer la robustesse
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const defaultConfig = {
        ...
        minRefreshDelay: 15000,     // 15 secondes
        debounceDelay: 500,
        baseApiUrl: '/api',
        fetchTimeoutDuration: 20000, // 20 secondes
        maxErrorsBeforeThrottle: 5,
        errorThrottleDelay: 60000,  // 1 minute
        useSimulatedDataOnError: true, // Utiliser des données simulées si l'API échoue
        pollingEnabled: true,
        activeMode: 'polling', // 'polling' ou 'socketio' (pour future extension)
        themeColors: {
        ...
        serviceColors: {
            ...
            'Informatique': '#607D8B',
            'RH': '#FF9800',
            'Commerce Anecoop-Solagora': '#FFC107', // Nom complet du service
            'Marketing': '#9C27B0',
            'Florensud': '#4CAF50',
            'Comptabilité': '#2196F3',
            'default': '#7a7574'
    ...
    if (config.debugMode) {
        console.log('Dashboard Core: Initializing (v2.0.0)');
    ...
    const dashboardState = {
        ...
        apiData: { sessions: [], participants: [], activites: [] }, // Données de l'API
        displayData: { sessions: [], participants: [], activites: [] }, // Données utilisées pour l'affichage (peut être simulé)
        errorCount: 0,
    ...
    const errorHandler = window.apiErrorHandler || {
        handleApiError: (endpoint, errorData, statusCode) => {
            ...
            return false; // Indique que l'erreur n'a pas été "récupérée"
        },
    ...
    function getSimulatedData() {
        // Données simulées (similaires à celles de api-dashboard-essential.js)
        return {
            sessions: [
                ...
                { id: 6, date: "mardi 3 juin 2025", horaire: "10h45–12h15", theme: "Gérer mes fichiers (OneDrive/SharePoint)", theme_id: 3, places_restantes: 0, inscrits: 10, max_participants: 10, liste_attente: 2, salle: "Salle Tramontane", salle_id: 1 },
                { id: 7, date: "mardi 3 juin 2025", horaire: "14h00–15h30", theme: "Collaborer avec Teams", theme_id: 4, places_restantes: 5, inscrits: 5, max_participants: 10, liste_attente: 0, salle: "Salle Tramontane", salle_id: 1 },
                { id: 8, date: "mardi 3 juin 2025", horaire: "15h45–17h15", theme: "Collaborer avec Teams", theme_id: 4, places_restantes: 5, inscrits: 5, max_participants: 10, liste_attente: 0, salle: "Salle Tramontane", salle_id: 1 }
            ],
            participants: [
        ...
        try {
            ...
            if (sessions && Array.isArray(sessions)) {
                ...
                sessions.forEach(session => {
                    ...
                    if (placesRestantes <= 0 && maxParticipants > 0) { // Session complète si places restantes <= 0 ET qu'il y a une capacité définie
    ...
    function updateSessionTable(sessions) {
        try {
            ...
            sessions.forEach(session => {
                ...
                if (maxParticipants > 0) { // Seulement si une capacité est définie
                    if (placesRestantes <= 0) {
                        placesClass = 'text-danger fw-bold'; placesIcon = 'fa-times-circle';
                    } else if (placesRestantes <= Math.floor(maxParticipants * 0.25)) { // Moins de 25% places
                        placesClass = 'text-warning'; placesIcon = 'fa-exclamation-triangle';
                    }
                } else { // Capacité non définie ou 0
            ...
            initTooltips(sessionTableBody); // Initialize tooltips for new elements
        } catch (e) {
    ...
    function updateActivityFeed(activities) {
        try {
            const container = document.getElementById('recent-activity-list'); // Cible la liste ul/div
            if (!container) {
    ...
    function updateThemeDistributionChart(sessions) {
        ...
        if (!sessions || sessions.length === 0) {
            ...
            noDataMsg.innerHTML = '<i class="fas fa-chart-pie fa-2x mb-2"></i><p>Aucune donnée d\'inscription par thème disponible pour le moment.</p>';
        ...
        if (labels.length === 0) { // Si tous les comptes sont à 0
            const noDataMsg = document.createElement('div');
            noDataMsg.className = 'no-data-message-chart text-center p-3 text-muted';
            noDataMsg.innerHTML = '<i class="fas fa-chart-pie fa-2x mb-2"></i><p>Aucune inscription enregistrée pour les thèmes actuels.</p>';
            canvas.parentNode.appendChild(noDataMsg);
            return;
        }
    ...
    function updateServiceDistributionChart(participants) {
        ...
        if (!participants || participants.length === 0) {
            const noDataMsg = document.createElement('div');
            noDataMsg.className = 'no-data-message-chart text-center p-3 text-muted';
            noDataMsg.innerHTML = '<i class="fas fa-chart-bar fa-2x mb-2"></i><p>Aucune donnée de participant par service disponible.</p>';
            canvas.parentNode.appendChild(noDataMsg);
            return;
        }
        ...
        if (labels.length === 0) {
             const noDataMsg = document.createElement('div');
            noDataMsg.className = 'no-data-message-chart text-center p-3 text-muted';
            noDataMsg.innerHTML = '<i class="fas fa-chart-bar fa-2x mb-2"></i><p>Aucun participant enregistré pour les services actuels.</p>';
            canvas.parentNode.appendChild(noDataMsg);
            return;
        }
    ...
    async function fetchAndProcessData(forceRefresh = false, isLightPoll = false) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                try { errorData = await response.json(); } catch (e) { /* ignore json parsing error */ }
            ...
            dashboardState.apiData = data; // Stocker les données brutes de l'API
            ...
            return true; // Indique que la mise à jour a eu lieu (ou a été tentée avec succès)
        } catch (error) {
            ...
            if (!recovered && config.useSimulatedDataOnError) {
                ...
                processAndRenderData(dashboardState.displayData, true); // Force refresh avec données simulées
            ...
            return false; // Indique que la mise à jour a échoué ou utilise un fallback
        } finally {
            ...
            if (!dashboardState.initialLoadComplete) {
                ...
                showGlobalLoading(false); // S'assurer que le loader est caché après le premier chargement
    ...
    function processAndRenderData(dataToRender, forceRefresh) {
        ...
        try {
            ...
            // Participants (pour les graphiques, pas d'affichage direct de liste ici)
            ...
            if (participantsHash !== dashboardState.dataHashes.participants || forceRefresh) {
                ...
                hasChangedOverall = true; // Marquer comme changé pour que les graphiques se mettent à jour
            }
            ...
            // Mettre à jour les graphiques si quelque chose a changé ou si c'est un forceRefresh
        ...
        // Appliquer les corrections UI après le rendu
        setTimeout(() => {
    ...
    function startPolling() {
        ...
        clearTimeout(dashboardState.pollingTimeoutId); // Clear any existing timeout
        ...
        const poll = async () => {
            if (document.visibilityState === 'visible' && !dashboardState.isUpdating && !dashboardState.isErrorThrottleActive) {
                ...
                await fetchAndProcessData(false); // false = not a forced full refresh
            }
            // Reschedule only if polling is still active
            if (dashboardState.pollingActive) {
                 dashboardState.pollingTimeoutId = setTimeout(poll, config.autoRefreshInterval);
            }
        };
        // Initial call to start the polling loop, then it reschedules itself
        dashboardState.pollingTimeoutId = setTimeout(poll, config.autoRefreshInterval);
    ...
    function setupEventListeners() {
        ...
        if (refreshButton) {
            refreshButton.addEventListener('click', () => {
                ...
                fetchAndProcessData(true); // true = force full refresh
            });
        }

        // Écouter les changements de visibilité pour optimiser le polling
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'visible' && dashboardState.pollingActive) {
                ...
                fetchAndProcessData(false); // Refresh on becoming visible
                startPolling(); // Ensure polling restarts if it was implicitly paused
            } else if (document.visibilityState === 'hidden' && dashboardState.pollingActive) {
                 if (config.debugMode) console.log("Dashboard Core: Page hidden, polling will be less frequent or paused by browser.");
                 // Le navigateur peut ralentir les timers, pas besoin de stopPolling explicitement ici
            }
        });
    }
    ...
    async function initializeDashboard() {
        ...
        setupEventListeners();
        // Pas besoin de setupMutationObserver pour l'instant
        // Pas besoin de initializeCharts ici, car ils sont créés/mis à jour dans processAndRenderData

        await fetchAndProcessData(true); // Premier chargement forcé

        if (dashboardState.pollingActive) {
            startPolling();
        }
        // Le loader est caché et l'événement 'dashboardCoreFullyReady' est émis dans fetchAndProcessData
    }

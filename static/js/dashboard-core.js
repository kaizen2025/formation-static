...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function getSimulatedData() {
        ...
        return {
            sessions: [
                { id: 1, date: "mardi 13 mai 2025", horaire: "09h00–10h30", theme: "Communiquer avec Teams", theme_id: 1, places_restantes: 0, inscrits: 10, max_participants: 10, liste_attente: 0, pending_validation_count: 1, salle: "Salle Tramontane", salle_id: 1 },
                { id: 2, date: "mardi 13 mai 2025", horaire: "10h45–12h15", theme: "Communiquer avec Teams", theme_id: 1, places_restantes: 1, inscrits: 9, max_participants: 10, liste_attente: 0, pending_validation_count: 0, salle: "Salle Tramontane", salle_id: 1 },
            ],
            participants: [
                { id: 1, nom: "PHILIBERT", prenom: "Elodie", email: "e.philibert@example.com", service: "Qualité", service_id: "qualite", service_color: config.serviceColors['Qualité'] },
                { id: 4, nom: "BIVIA", prenom: "Kevin", email: "k.bivia@example.com", service: "Informatique", service_id: "informatique", service_color: config.serviceColors['Informatique'] },
            ],
            ...
            services: [ {id: "qualite", nom: "Qualité"}, {id: "informatique", nom: "Informatique"}, {id: "rh", nom: "RH"} ],
            salles: [ {id: 1, nom: "Salle Tramontane", capacite: 10}, {id:2, nom: "Salle Mistral", capacite: 8} ],
    ...
    function updateSessionTable(sessions = []) {
        ...
        sessions.forEach(session => {
            ...
            let placesClass = 'text-success'; let placesIcon = 'fa-check-circle';
            if (maxParticipants > 0) {
                if (placesRestantes <= 0) { placesClass = 'text-danger fw-bold'; placesIcon = 'fa-times-circle'; }
                else if (placesRestantes <= Math.floor(maxParticipants * 0.25)) { placesClass = 'text-warning'; placesIcon = 'fa-exclamation-triangle';}
            } else { placesClass = 'text-muted'; placesIcon = 'fa-minus-circle'; }
            ...
            const rowHtml = `
                <tr class="session-row" data-session-id="${session.id || ''}">
                    ...
                    <td class="theme-cell"><span class="theme-badge" style="background-color: ${themeColor}; color: white;" data-bs-toggle="tooltip" title="${themeName}">${themeName}</span></td>
                    <td class="places-dispo text-nowrap ${placesClass} text-center"><i class="fas ${placesIcon} me-1"></i> ${placesRestantes} / ${maxParticipants || '∞'}</td>
                    <td class="js-salle-cell">${session.salle ? `<span class="badge bg-light text-dark border">${session.salle}</span>` : '<span class="badge bg-secondary">Non définie</span>'}</td>
                    <td class="text-nowrap text-center">
                        ...
                        <button type="button" class="btn btn-sm btn-primary me-1 action-btn btn-inscrire-session" data-bs-toggle="modal" data-bs-target="#genericInscriptionModal" data-session-id="${session.id || ''}" title="Inscrire un participant"><i class="fas fa-plus"></i></button>
                        ${(window.dashboardConfig && window.dashboardConfig.isAdmin) ? `<button type="button" class="btn btn-sm btn-outline-info action-btn btn-attribuer-salle" data-bs-toggle="modal" data-bs-target="#genericAttribuerSalleModal" data-session-id="${session.id || ''}" title="Attribuer Salle"><i class="fas fa-building"></i></button>` : ''}
                    </td>
                </tr>`;
    ...
    async function fetchAndProcessData(forceRefresh = false) {
        ...
        try {
            ...
            if (!recovered && config.useSimulatedDataOnError) {
                ...
                processAndRenderData(getSimulatedData(), true); // Afficher au moins la structure
            }

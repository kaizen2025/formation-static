...
function updateElementText(elementId, value, defaultValue = '0') {
    ...
    if (element) {
        // Supprimer les spinners enfants s'ils existent
...
function updateStatisticsCounters(sessions = []) {
    try {
        ...
        // ID ajustés pour correspondre au HTML du dashboard fourni précédemment
...
function updateSessionTable(sessions = []) {
    ...
    sessionTableBody.innerHTML = ''; // Vider les anciennes lignes

    if (!Array.isArray(sessions) || sessions.length === 0) {
        const cols = sessionTableBody.closest('table')?.querySelectorAll('thead th').length || 5;
        sessionTableBody.innerHTML = `<tr class="no-data-row"><td colspan="${cols}" class="text-center p-4 text-muted"><i class="fas fa-info-circle me-2"></i>Aucune session à venir à afficher pour le moment.</td></tr>`;
        return;
    }
    ...
    initTooltips(sessionTableBody); // Réinitialiser les tooltips pour les nouveaux éléments
}
...
function createChart(canvasId, type, data, options) {
    ...
    if (!canvas) { if (config.debugMode) console.warn(`createChart: Canvas with ID '${canvasId}' not found.`); return null; }
    const ctx = canvas.getContext('2d');
    const parentContainer = canvas.parentNode;
    parentContainer.querySelector('.no-data-message-chart')?.remove(); // Enlever ancien message

    if (!data || !data.labels || data.labels.length === 0 || !data.datasets || data.datasets.some(ds => !ds.data || ds.data.length === 0 || ds.data.every(val => val === 0) )) {
        ...
        noDataMsg.innerHTML = `<i class="fas ${type === 'doughnut' ? 'fa-chart-pie' : 'fa-chart-bar'} fa-2x mb-2 text-gray-300"></i><p class="mb-0">Aucune donnée à afficher.</p>`;
...
function updateThemeDistributionChart(sessions = []) {
    ...
    const labels = Object.keys(themeCounts).filter(k => themeCounts[k] > 0); // Seulement les thèmes avec inscriptions
    ...
    dashboardState.themeChartInstance = createChart('themeDistributionChart', 'doughnut',
        { labels: labels, datasets: [{ data: dataValues, backgroundColor: backgroundColors, borderWidth: 1, hoverOffset: 4 }] },
        { responsive: true, maintainAspectRatio: false, cutout: '65%', plugins: { legend: { position: 'bottom', labels: { font: { size: 11 }, boxWidth: 12, padding: 10, usePointStyle: true } }, tooltip: { callbacks: { label: (c) => { const total = c.dataset.data.reduce((a,b)=>a+b,0); return ` ${c.label}: ${c.raw} (${(total > 0 ? (c.raw / total * 100) : 0).toFixed(0)}%)`;} } } } }
    );
}
...
function updateServiceDistributionChart(participants = []) {
    ...
    dashboardState.serviceChartInstance = createChart('serviceDistributionChart', 'bar',
        { labels: labels, datasets: [{ label: 'Participants', data: dataValues, backgroundColor: backgroundColors, borderWidth: 0, barThickness: 'flex', maxBarThickness: 30 }] },
...
function initTooltips(parentElement = document) {
    if (typeof bootstrap !== 'undefined' && bootstrap.Tooltip) {
        ...
        tooltipTriggerList.forEach(el => {
            ...
            if (existingTooltip) existingTooltip.dispose(); // Détruire l'ancien pour éviter les doublons
...
function populateSelectWithOptions(selectElement, optionsArray, valueField, textField, placeholder = "-- Choisir --", currentVal = null) {
    if (!selectElement || !Array.isArray(optionsArray)) {
        if(config.debugMode && !selectElement) console.warn("populateSelect: selectElement not found for placeholder:", placeholder);
        if(config.debugMode && !Array.isArray(optionsArray)) console.warn("populateSelect: optionsArray is not an array", optionsArray);
        if(selectElement) selectElement.innerHTML = `<option value="">${placeholder} (Erreur chargement)</option>`;
        return;
    }
...
function setupGenericModals() {
    ...
    if (participantsModalEl) {
        participantsModalEl.addEventListener('show.bs.modal', async function(event) {
            ...
            if (spinner) spinner.style.display = 'flex'; if (tabContent) tabContent.style.display = 'none';
...
window.dashboardCore = {
    ...
    getState: () => ({ ...dashboardState }), // Retourne une copie
    getConfig: () => ({ ...config })
};

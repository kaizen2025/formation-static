...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const defaultConfig = {
        ...
        pollingEnabled: true,
        activeMode: 'polling',
        themeColors: { 'Communiquer avec Teams': '#464eb8', 'Gérer les tâches (Planner)': '#038387', 'Gérer mes fichiers (OneDrive/SharePoint)': '#0078d4', 'Collaborer avec Teams': '#107c10', 'default': '#6c757d' },
        serviceColors: { 'Qualité': '#F44336', 'Informatique': '#607D8B', 'RH': '#FF9800', 'Commerce Anecoop-Solagora': '#FFC107', 'Marketing': '#9C27B0', 'Florensud': '#4CAF50', 'Comptabilité': '#2196F3', 'default': '#7a7574' }
    };
    ...
    function getSimulatedData() { /* ... (Contenu de getSimulatedData comme dans v2.2.2) ... */ 
    ...
    function updateElementText(elementId, value, defaultValue = '0') { const element = document.getElementById(elementId); if (element) { element.textContent = (typeof value === 'number' ? value.toLocaleString() : value) || defaultValue; } }
    ...
    function setupGenericModals() {
        ...
        if (participantsModalEl) {
            participantsModalEl.addEventListener('show.bs.modal', async function(event) {
                ...
                if (spinner) spinner.style.display = 'block'; if (tabContent) tabContent.style.display = 'none';
                ...
                try {
                    ...
                    if (!response.ok) throw new Error(`Erreur ${response.status} chargement détails`);
                    const details = await response.json(); if (details.status !== 'ok') throw new Error(details.message || "Erreur API détails");

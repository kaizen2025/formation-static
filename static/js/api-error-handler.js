...
class ApiErrorHandler {
    ...
    handleMaxParticipantsError() { /* Similaire à places_restantes */ return true; }
    handleTypeError() { /* Logique de récupération pour TypeError */ return true; }
    handleUndefinedError() { /* Logique de récupération pour undefined */ return true; }
    ...
    // reportErrorToServer(endpoint, errorData, errorType) { ... } // Optionnel
        ...
        try {
            ...
            if (url && url.includes('/api/') && !response.ok) {
                ...
                try {
                    ...
                    try {
                        errorData = { message: await response.clone().text() }; // Essayer de récupérer le texte si pas JSON
                    } catch (textErr) {
            ...
            return response;
        } catch (error) { // Erreur réseau (pas de réponse du serveur) ou AbortError
            if (url) { // S'assurer qu'on a une URL
